...
#def invparameters(RelativeError, MarquardtScheme, 
#                  DeltaPhiAbortPercent, MaxIter):
#def invparameters():
...
def invparameters(RelativeError, MarquardtScheme, 
                  DeltaPhiAbortPercent, MaxIter):
    invEM.setRelativeError(RelativeError)
    invEM.setMarquardtScheme(MarquardtScheme)
    invEM.setDeltaPhiAbortPercent(DeltaPhiAbortPercent)
    invEM.setMaxIter(MaxIter)
    # Finalisation
    modelEM = invEM.run()
    respEM = invEM.response()
...
RE = [0.1/100, 0.2/100, 0.2/100]
MS = [0.8, 0.7, 0.5]
DPAP = [0.00001, 0.0001, 0.0005]
MI = [40, 40, 40]
...
#        results = [executor.submit(do_something, sec) for sec in secs]
        results = executor.map(invparameters, RE, MS, DPAP, MI)
